...
#                 loss += self.compute_distillation_loss(scores, scores_teach)
...
class DistilationModel(nn.Module):
    ...
    def forward(self, query: Dict[str, Tensor] = None, passage: Dict[str, Tensor] = None, teacher_score: Tensor = None):
        ...
        if self.training:
            ...
            if self.distil_loss == True:
                q_reps_teacher = self.encode(query)
                p_reps_teacher = self.encode(passage)


                # Compute softmax probabilities from logits
                q_softmax_student = F.softmax(q_reps / self.temperature, dim=-1)
                p_softmax_student = F.softmax(p_reps / self.temperature, dim=-1)
                q_softmax_teacher = F.softmax(q_reps_teacher / self.temperature, dim=-1)
                p_softmax_teacher = F.softmax(p_reps_teacher / self.temperature, dim=-1)

                distillation_loss = F.kl_div(q_softmax_student.log(), q_softmax_teacher, reduction='batchmean') \
                                    + F.kl_div(p_softmax_student.log(), p_softmax_teacher, reduction='batchmean')
