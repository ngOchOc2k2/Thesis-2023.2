...
class Prompt(nn.Module):
    ...
    def forward(self, x_embed, x_key=None):
        ...
        similarity = torch.matmul(x_key_norm, prompt_key_norm.t())
        prompt_weights = F.softmax(similarity, dim=1)

        # Calculate prompt as the weighted average of prompts
        prompt_weights_expanded = prompt_weights.unsqueeze(2)  # Shape: (batch_size, pool_size, 1)
        batched_prompt = torch.sum(prompt_weights_expanded * self.prompt, dim=1)  # Shape: (batch_size, length, embed_dim)

        # Put pull_constraint loss calculation inside
        x_key_norm = x_key_norm.unsqueeze(1)
        out["reduce_sim"] = torch.sum(prompt_key_norm * x_key_norm) / x_embed.shape[0]
        out["prompted_embedding"] = torch.cat([batched_prompt, x_embed], dim=1)
        return out