...
PROMPT_TASK_TACRED = """There are four classes: {re1}, {re2}, {re3}, {re4}, each representing different types of relationships that can exist between the two entities. 
{relation}
Example the {relation_name}: {example}
"""


PROMPT_TASK_FEWREL = """There are eight classes: {re1}, {re2}, {re3}, {re4}, {re5}, {re6}, {re7}, {re8} each representing different types of relationships that can exist between the two entities. 
{relation}
Example the {relation_name}: {example}
"""
...
PROMPT_NEGATIVE = """ There are four classes: place served by transport hub, located on terrain feature, musical conductor, participating team, each representing different types of relationships that can exist between the two entities. 
{relation}
Example the {relation_name}: {example}
"""
...
def get_description(task, example, relation_type, description, data_type, id2rel):
    if data_type == 'TACRED':
        ...
        'relation_name': id2rel[example['relation']],
...
def prepare_data_for_retrieval(config, steps, bge_m3, current_relation, description, current_data, data_memory, id2rel):
    ...
    if steps == 0:
        ...
        no_relation_text = [PROMPT_NEGATIVE.format_map({
            ...
            'relation_name': id2rel[item['relation']],
            ...
            for task_neg in range(len(memory_data)):
                if task != task_neg:
                    for re_task, samples in memory_data[task_neg]['data'].items():
                        neg_query += [get_description(steps, sample, memory_data[task_neg]['relations_task'], description, config.task_name, id2rel) for sample in samples]
    ...
    return data_train, data_last_train, f'/train_step_{steps}.jsonl', f'/train_last_{steps}.jsonl', '/kaggle/working/model_bge'
...
if __name__ == '__main__':
    ...
    for rou in range(config.total_round):
        ...
        for steps, (training_data, valid_data, test_data, current_relations, historic_test_data, seen_relations) in enumerate(sampler):
            ...
            if config.trainable_retrieval:
                if steps > 0:
                    train_retrieval(config=config, data_path=path_last, model_path='/kaggle/working/model_bge')
                    train_retrieval(config=config, data_path=path_total, model_path='/kaggle/working/model_bge')
                else:
