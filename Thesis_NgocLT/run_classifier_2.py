...
def evaluate_strict_all(config, steps, retrieval_path, test_data_all, memories_data, list_map_relid2tempid, description, id2rel):    
    ...
    if steps == 0:      
        ...
        data_loader = get_data_loader(config, test_data_all[steps]['data'], batch_size=1)

        encoder.eval()
        classifier.eval()
        n = len(test_data_all[steps]['data'])
        ...
        for step, batch_data in enumerate(data_loader):
            ...
            seen_relation_ids = [rel2id[relation] for relation in test_data_all[steps]['seen_relation']]
        ...
        result_classifier.append(correct / n)
        result_total.append(correct / n)
    ...
    # if task > 0
            ...
            for keys, values in enumerate(task['data']):
                for sample in task['data'][values]:
                    if config.task_name == 'TACRED':
                        memories_data_text.append(PROMPT_TASK_TACRED_ALL.format_map({
                                're1': task['relations_task'][-4],
                                're2': task['relations_task'][-3],
                                're3': task['relations_task'][-2],
                                're4': task['relations_task'][-1],
                                
                                'relation1': description[task['relations_task'][-4]],
                                'relation2': description[task['relations_task'][-3]],
                                'relation3': description[task['relations_task'][-2]],
                                'relation4': description[task['relations_task'][-1]],
        ...
        embedding_memories_data = bge_model.encode(memories_data_text, max_length=512, return_dense=True, return_colbert_vecs=True)
        ...
        for task, test_data in enumerate(test_data_all):
            ...
            if config.type_similar == 'colbert':
                # embedding test data
                embedding_test_data = bge_model.encode(test_data_text, max_length=128, return_dense=True, return_colbert_vecs=True)
...
def prepare_data_for_retrieval(config, steps, bge_m3, current_relation, description, current_data, memory_data, id2rel):
    ...
    if steps == 0:
        ...
        embedding_query = retrieval_model.encode(query, max_length=128, return_dense=True, return_colbert_vecs=True)
        embedding_neg = retrieval_model.encode(no_relation_text, max_length=512, return_dense=True, return_colbert_vecs=True)
            ...
            for idx_query, query_text in enumerate(query):
                ...
                for sample in current_data:
                    if sample['text'] != query_text and sample['relation'] == task_relation[idx_query]:
                        positive = PROMPT_TASK_TACRED_ALL.format_map({
                            ...
                            'relation1': description[current_relation[0]],
                            'relation2': description[current_relation[1]],
                            'relation3': description[current_relation[2]],
                            'relation4': description[current_relation[3]],
                            'example_relation': id2rel[sample['relation']],
                            'relation': description[id2rel[sample['relation']]],
                            'example': sample['text']
                        })
        ...
        for task in range(len(memory_data)):
            current_query, current_relation, original_this_task, neg_query = [], [], [], []
            ...
            for keys, re_task in enumerate(memory_data[task]['data']):
                ...
                current_relation += [item['relation'] for item in memory_data[task]['data'][re_task]]
            ...
            for task_neg in range(len(memory_data)):
                if task != task_neg:
                    ...
                    for keys, re_task in enumerate(memory_data[task_neg]['data']):
                        neg_query += [PROMPT_TASK_TACRED_ALL.format_map({
                                # 're1': neg_relation[0],
                                # 're2': neg_relation[1],
                                # 're3': neg_relation[2],
                                # 're4': neg_relation[3],
                                ...
                                'relation1': description[neg_relation[0]],
                                'relation2': description[neg_relation[1]],
                                'relation3': description[neg_relation[2]],
                                'relation4': description[neg_relation[3]],
                                'example_relation': id2rel[sample['relation']],
                                'relation': description[id2rel[sample['relation']]],
                                'example': sample['text']
                        }) for sample in memory_data[task_neg]['data'][re_task]]

            embedding_query = retrieval_model.encode(current_query, max_length=128, return_dense=True, return_colbert_vecs=True)
            embedding_neg = retrieval_model.encode(neg_query, max_length=512, return_dense=True, return_colbert_vecs=True)  
            ...
            if config.type_similar == 'colbert':
                ...
                for idx_query, query_text in enumerate(current_query):
                    ...
                    for sample in original_this_task:
                        if sample['text'] != query_text and sample['relation'] == current_relation[idx_query]:
                            positive = PROMPT_TASK_TACRED_ALL.format_map({
                                ...
                                'relation1': description[this_list_relation[0]],
                                'relation2': description[this_list_relation[1]],
                                'relation3': description[this_list_relation[2]],
                                'relation4': description[this_list_relation[3]],
                                'example_relation': id2rel[sample['relation']],
                                'relation': description[id2rel[sample['relation']]],
                                'example': sample['text']
                            })
    ...
    for rou in range(config.total_round):
        ...
        for steps, (training_data, valid_data, test_data, current_relations, historic_test_data, seen_relations) in enumerate(sampler):
            ...
            for relation in current_relations:
                ...
                # print(f"Length data of {relation}: {len(this_task_memory[relation])}")
